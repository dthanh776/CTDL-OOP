...
namespace SinhVienGhiFIle
{
    ...
    class Bai3
    {
        ...
        static void XuatDSSV(SinhVien[] arr)
        {
            for (int i = 0; i < arr.Length; i++)
            {
                Console.WriteLine($"{"\tMaSV: ",-20}{arr[i].maSV}");
                Console.WriteLine($"{"\tHo ten: ",-20}{arr[i].hoTen}");
                Console.WriteLine($"{"\tNgay Sinh: ",-20}{arr[i].ngaySinh}");
                Console.WriteLine($"Danh Sach: {arr[i].maSV} mon da hoc");
                Console.WriteLine($"{"MaMH",-15}{"TenMH",-15}{"SoTC",-15}{"DiemTB",-15}");
                for (int k = 0; k < arr[i].arrMH.Length; k++)
                {
                    Console.WriteLine($"{arr[i].arrMH[k].maMH,-15}{arr[i].arrMH[k].tenMH,-15}{arr[i].arrMH[k].soTC,-15}{arr[i].arrMH[k].diem,-15}");
                }
                Console.WriteLine($"Diem trung binh: {Math.Round(TinhTB(arr[i]), 2)}");
                Console.WriteLine("*********************");
            }
        }
        ...
        static void InterchangeSort(SinhVien[] arr) //Sap xep theo ho ten
        {
            for (int i = 0; i < arr.Length - 1; i++)
            {
                for (int j = i + 1; j < arr.Length; j++)
                {
                    if (String.Compare(arr[j].hoTen, arr[i].hoTen) == -1)
                    {
        ...
        static void SelectionSort(SinhVien[] arr) // Sap xep theo diem trung binh
        {
            int min;
            for (int i = 0; i < arr.Length - 1; i++)
            {
